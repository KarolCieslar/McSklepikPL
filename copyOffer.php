	...
	if (!empty($_POST)){
		if (!empty($_POST['name']) && !empty($_POST['server']) && !empty($_POST['desc']) && !empty($_POST['price'])) {
				...
				if($_FILES['fileToUpload']['error'] > 0) { 
					...
					$rand = rand(10,900000);
					$newLinkIMG = ''.$rand.'x'.$linkIMG.'';
					copy('../uploads/offerts/'.$linkIMG.'','../uploads/offerts/'.$newLinkIMG.'');
				} else {
					$rand = rand(10,900000);
				...
				if ($uploadOk == 1) {
					$blad = '<div class="alert alert-success"> <span>Nowa oferta na podstawie tej oferty została dodana pomyślnie! Przejdź do zakładki USŁUGI aby nią zarządzać!</span></div>';
					mysql_result(mysql_query("INSERT INTO `shop`.`logs` (`uid`, `date`, `info`, `type`, `licenseID`) VALUES  (NULL, CURRENT_TIMESTAMP, 'Dodano nową ofertę <b>$name</b> na podstawie oferty <b>$name</b>', 'copyoffer', '$licenseID');"), 0 );
					mysql_result(mysql_query("INSERT INTO `shop`.`products` (`uid`, `name`, `server`, `commands`, `img`, `price`, `category`, `description`, `serverUID`, `licenseID`) VALUES (NULL, '$name', '$server', '$cmds', '$newLinkIMG', '$price', '$categoryUID', '$desc', '$serverUID', '$licenseID');"), 0 );
				}
                    ...
                    <div class="col-md-14">
                        <div class="card">
                            <div style="padding-bottom:10px;" class="header col-md-10">
                                <h4 class="title">Kopiuj usługę</h4>
                                <p class="category">Uzupełnij pola poniżej aby skopiować usługę</p>
                            </div>
                                    ...
                                    <div class="row">
                                        ...
                                        <div style="margin-top:26px" class="col-md-5">
                                            <div class="form-group">
                                                ...
                                                <input maxlength="10" name="price" value="<?php echo mysql_result( mysql_query("SELECT price FROM `products` WHERE `uid` = $productUID"), 0 ); ?>" min="1" type="number" placeholder="Podaj cene produktu!" class="form-control border-input" required>
                                            </div>
                                        </div>
									</div>
